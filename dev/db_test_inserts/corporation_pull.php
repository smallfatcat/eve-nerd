...
try {
    ...
    $sql = "SELECT DISTINCT corporation_id FROM attackers WHERE corporation_id NOT IN (SELECT corporation_id FROM corporations) ORDER BY corporation_id LIMIT ".$batchSize;
    ...
    $csql = $conn->prepare("INSERT INTO corporations (corporation_id, corporation_name) VALUES (?, ?)");
    ...
    while($counter < $queue->rowCount()) {
        ...
        for($i = 0; ($i < $batchSize && $counter < $queue->rowCount()); $i++)
        {
            ...
            //if($row["character_id"]!=0){
                    $char_list =  $row["corporation_id"].($i==0?'':',').$char_list;
            //}
        }
        echo $char_list;
