...
try {
    ...
    $sql = "SELECT DISTINCT alliance_id FROM attackers WHERE alliance_id NOT IN (SELECT alliance_id FROM alliances) ORDER BY alliance_id LIMIT ".$batchSize;
    ...
    $csql = $conn->prepare("INSERT INTO alliances (alliance_id, alliance_name) VALUES (?, ?)");
    ...
    while($counter < $queue->rowCount()) {
        ...
        for($i = 0; ($i < $batchSize && $counter < $queue->rowCount()); $i++)
        {
            ...
            //if($row["character_id"]!=0){
                    $char_list =  $row["alliance_id"].($i==0?'':',').$char_list;
            //}
        }
        echo $char_list;
