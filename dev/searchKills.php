<?php
	if(isset($_GET["solar_system_id"])){
		...
		$ksql = $conn->prepare("SELECT killmail_id, typeName, character_name, killmail_time FROM nerdDB.killmails JOIN eve_sde.invTypes ON nerdDB.killmails.ship_type_id = eve_sde.invTypes.typeID JOIN nerdDB.characters ON nerdDB.killmails.character_id = nerdDB.characters.character_id WHERE solar_system_id = ?");
